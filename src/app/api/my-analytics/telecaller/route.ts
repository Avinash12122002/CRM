...
export async function GET(req: NextRequest) {
  try {
    ...
    if (payload.role !== "telecaller" && payload.role !== "employee" && payload.role !== "wtc" && payload.role !== "supervisor" && payload.role !== "follow_up" && payload.role !== "trainee") {
    ...
    const meetingReschedule = cohort.filter((l) => l.status === "meeting-reschedule").length;
    ...
    return NextResponse.json({
      ...
      filtered: isFiltered,
      totalInDb,
      metrics: { totalLeads, newLeads, inProgress, meetingScheduled, meetingReschedule, lost, sales, conversionRate, dropRate },
      statusDistribution,
      callbacksDueToday,
      dailyTrend,
      history: {
