...
export async function PUT(
  ...
  try {
    ...
    if ((lead.country || "") !== (country || ""))
      changes.push(
        `Country: "${lead.country || "N/A"}" → "${country || "N/A"}"`,
      );
    if ((lead.state || "") !== (state || ""))
    ...
    if ((lead.passportType || "") !== (passportType || ""))
      changes.push(
        `Passport Type: "${lead.passportType || "N/A"}" → "${passportType || "N/A"}"`,
      );
    ...
    if (oldCallbackDate !== newCallbackDate) {
      changes.push(
        `Callback Date: "${oldCallbackDate || "N/A"}" → "${newCallbackDate || "N/A"}"`,
      );
    }
    ...
    await logUserAction(db, {
      ...
      entityId: leadId,
      summary: `Updated Triloknath lead #${leadId} (${lead.name || "Candidate"}): ${changes.length > 0 ? changes.slice(0, 3).join("; ") : "details updated"}`,
