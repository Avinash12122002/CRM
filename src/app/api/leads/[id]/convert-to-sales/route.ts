...
export async function POST(
  ...
  try {
    ...
    await logUserAction(db, {
      ...
      entityType: "lead",
      entityId: leadId,
      summary: `Converted candidate "${lead.name || `#${leadId}`}" to Sales with signed document and handed over to Case Manager ${caseManager.name}`,
      metadata: {
        leadName: lead.name,
        caseManagerName: caseManager.name,
        occupations,
    ...
    try {
      ...
      if (assignedTrainee) {
        await createNotification({
          ...
          message: `Lead ${lead.name || `#${leadId}`} was converted to Sales and assigned to you.`,
          type: "lead_assigned",
